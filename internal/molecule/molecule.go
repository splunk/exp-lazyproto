...
// MessageEach iterates over each top-level field in the message stored in buffer
// and calls fn on each one.
func MessageFieldNums(buffer *codec.Buffer, fn func(fieldNum int32)) error {
	for !buffer.EOF() {
		v, err := buffer.DecodeVarint()
		if err != nil {
			return err
		...
		case codec.WireFixed32:
			_, err = buffer.DecodeFixed32()
		case codec.WireFixed64:
			_, err = buffer.DecodeFixed64()
		case codec.WireBytes:
			_, err = buffer.DecodeRawBytes(false)
		case codec.WireStartGroup, codec.WireEndGroup:
			err = fmt.Errorf(
				"MessageEach: encountered group wire type: %d. Groups not supported",
				wireType,
...
func PackedRepeatedEach(
	buffer *codec.Buffer, fieldType codec.FieldType, fn PackedRepeatedEachFn,
) error {
	var wireType codec.WireType
	switch fieldType {
	...
	for !buffer.EOF() {
		...
		case codec.WireStartGroup, codec.WireEndGroup:
			err = fmt.Errorf(
				"PackedRepeatedEach: encountered group wire type: %d. Groups not supported",
				wireType,
			)
		default:
		...
		if err != nil {
			return fmt.Errorf(
				"PackedRepeatedEach: error reading value from buffer: %v", err,
			)
		}

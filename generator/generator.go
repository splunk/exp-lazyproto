...
func (g *generator) prepareMessage() error {
	for _, field := range g.msg.Fields {
		...
		if field.GetType() == descriptor.FieldDescriptorProto_TYPE_MESSAGE {
			...
			flagName := fmt.Sprintf(
				"flag_%s_%s_Decoded", g.msg.GetName(), field.GetCapitalName(),
			)
	...
	for _, field := range g.msg.Fields {
		...
		if g.options.WithPresence {
			flagName := fmt.Sprintf(
				"flag_%s_%s_Present", g.msg.GetName(), field.GetCapitalName(),
			)
	...
	if g.msg.FlagsBitCount > 0 {
		...
		g.msg.FlagsType = fmt.Sprintf("%s_Flags", g.msg.GetName())
	}
